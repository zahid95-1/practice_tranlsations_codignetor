	...
	public function step_registrations(){
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			if($request == 'web'){
				...
				redirect(base_url() . 'register?reffid='.$_REQUEST['parent_id']);
			}else if($request == 'api'){
				print_r(json_encode($responseData,true));
				exit();
			}
			...
			if ($request=='api') {
				...
				redirect(base_url() . 'register-second-step?reffid=' . $_REQUEST['parent_id']);
			}
		}
	}
	...
	public function save_register_v2(){
		...
		if ($this->form_validation->run() == FALSE)
		{
			if($request == 'web'){
				...
				$_SESSION['error_new_registration']	  =json_encode($responseData,true);
				$_SESSION['request_data']			  =json_encode($_REQUEST,true);
				redirect(base_url() . 'register-second-step?reffid='.$_REQUEST['reffid']);
			}else if($request == 'api'){
				$responseData	=array(
					'status'    					=> 400,
					'first_name'					=>strip_tags(form_error('first_name')),
